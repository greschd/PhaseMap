...
if __name__ == '__main__':
    ...
    res = pm.get_phase_map(phase, [(-1.1, 1.1), (-1.1, 1.1)], num_steps=6, init_mesh=3)
    ...
    for i in range(B.shape[0]):
        ...
        if i % 2 == 1: iterator = reversed(iterator)
        for j in iterator:
            if B[i, j] == -1:
                B[i, j] = current
            else:
                current = B[i, j]
    color_mapping_1 = {-1: [0, 0, 0], 0: [0x00, 0x00, 0x00], 1: [0, 0x33, 0x99], 2: [0xee, 0x66, 0]}
    color_mapping_2 = {-1: [0, 0, 0], 0: [0xff, 0xff, 0xff], 1: [0, 0x1f, 0x5c], 2: [0xc2, 0x3b, 5]}
